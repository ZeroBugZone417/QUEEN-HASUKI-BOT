  ...
  hasuki.ev.on('messages.upsert', async ({ messages }) => {
    for (const msg of messages) {
      if (msg.messageStubType === 68) {
        await hasuki.sendMessageAck(msg.key);
      }
    }
    ...
    const groupMetadata = isGroup ? await hasuki.groupMetadata(from).catch(() => {}) : '';
    ...
    if (isCmd) {
      ...
      if (cmd) {
        if (cmd.react) hasuki.sendMessage(from, { react: { text: cmd.react, key: mek.key } });
        try {
          cmd.function(hasuki, mek, m, {
